...
const loginUser = async (payload) => {
    ...
    const accessToken = jwtHelpers.generateToken({
        ...
        role: userData.role,
    }, config.jwt_access_secret, config.jwt_access_expires_in);
...
const forgotPassword = async (payload) => {
    ...
    const resetPassToken = jwtHelpers.generateToken({ email: userData.email, userId: userData.id, role: userData.role }, config.reset_password_secret, config.reset_password_expires_in);
...
const getMe = async (user) => {
    ...
    const decodedData = jwtHelpers.verifyToken(accessToken, config.jwt_access_secret);
    const userData = await prisma.user.findUniqueOrThrow({
        where: {
            email: decodedData.email,
            status: UserStatus.ACTIVE,
